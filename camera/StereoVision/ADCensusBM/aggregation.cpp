...
Aggregation::Aggregation(const Mat &leftImage, const Mat &rightImage,
                         uint colorThreshold1, uint colorThreshold2,
...
int Aggregation::computeLimit(int height, int width, int directionH,
                              int directionW, uchar imageNo) {
  // reference pixel
  ...
  bool inside =
      (0 <= h1) && (h1 < imgSize.height) && (0 <= w1) && (w1 < imgSize.width);
  ...
  if (inside) {
    ...
    while (colorCond && wLimitCond && fColorCond && inside) {
      ...
      colorCond = colorDiff(p, p1) < colorThreshold1 &&
                  colorDiff(p1, p2) < colorThreshold1;

      // Is window limit not reached?
      wLimitCond = d < maxLength1;

      // Better color similarities for farther neighbors?
      fColorCond = (d <= maxLength2) ||
                   (d > maxLength2 && colorDiff(p, p1) < colorThreshold2);
      ...
      // test if p1 is still inside the picture
      inside = (0 <= h1) && (h1 < imgSize.height) && (0 <= w1) &&
               (w1 < imgSize.width);
...
Mat Aggregation::computeLimits(int directionH, int directionW, int imageNo) {
  ...
  int h, w;
#pragma omp parallel default(shared) private(w, h)                             \
    num_threads(omp_get_max_threads())
#pragma omp for schedule(static)
  for (h = 0; h < imgSize.height; h++) {
    for (w = 0; w < imgSize.width; w++) {
      limits.at<int>(h, w) =
          computeLimit(h, w, directionH, directionW, imageNo);
    }
  }
...
Mat Aggregation::aggregation1D(const Mat &costMap, int directionH,
                               int directionW, Mat &windowSizes,
                               uchar imageNo) {
...
#pragma omp parallel default(shared) private(w, h, d)                          \
    num_threads(omp_get_max_threads())
#pragma omp for schedule(static)
  for (h = 0; h < imgSize.height; h++) {
    for (w = 0; w < imgSize.width; w++) {
      ...
      for (d = dmin; d <= dmax; d++) {
        ...
        tmpWindowSizes.at<int>(h, w) +=
            windowSizes.at<int>(h + d * directionH, w + d * directionW);
      }
...
void Aggregation::aggregation2D(Mat &costMap, bool horizontalFirst,
                                uchar imageNo) {
  ...
  for (uchar direction = 0; direction < 2; direction++) {
    (aggregation1D(costMap, directionH, directionW, windowsSizes, imageNo))
        .copyTo(costMap);
    std::swap(directionH, directionW);
  }
