...
DisparityRefinement::DisparityRefinement(
    uint dispTolerance, int dMin, int dMax, uint votingThreshold,
...
Mat DisparityRefinement::outlierElimination(const Mat &leftDisp,
                                            const Mat &rightDisp) {
  ...
  for (int h = 0; h < dispSize.height; h++) {
    for (int w = 0; w < dispSize.width; w++) {
      ...
      // if the point is an outlier, differentiate it between occlusion and
      // mismatch
      if (w - disparity < 0 ||
          abs(disparity - rightDisp.at<int>(h, w - disparity)) >
              dispTolerance) {
        bool occlusion = true;
...
void DisparityRefinement::regionVoting(Mat &disparity,
                                       const vector<Mat> &upLimits,
                                       const vector<Mat> &downLimits,
                                       const vector<Mat> &leftLimits,
                                       const vector<Mat> &rightLimits,
...
void DisparityRefinement::properInterpolation(Mat &disparity,
                                              const Mat &leftImage) {
  Size dispSize = disparity.size();
  Mat dispTemp(dispSize, CV_32S);
      ...
      if (disparity.at<int>(h, w) >= dMin) {
        ...
        for (uchar direction = 0; direction < 16; direction++) {
          ...
          for (uchar sD = 0; sD < maxSearchDepth && inside && !gotDisp; sD++) {
            ...
            inside = hD >= 0 && hD < dispSize.height && wD >= 0 &&
                     wD < dispSize.width;
            if (inside && disparity.at<int>(hD, wD) >= dMin) {
              neighborDisps[direction] = disparity.at<int>(hD, wD);
              neighborDiffs[direction] = colorDiff(leftImage.at<Vec3b>(h, w),
                                                   leftImage.at<Vec3b>(hD, wD));
              gotDisp = true;
            }
          }
        }
        ...
        if (disparity.at<int>(h, w) == dMin - DISP_OCCLUSION) {
          ...
          for (uchar dir = 1; dir < 16; dir++) {
            if (minDiff < 0 ||
                (minDiff > neighborDiffs[dir] && neighborDiffs[dir] > 0)) {
...
void DisparityRefinement::discontinuityAdjustment(
    Mat &disparity, const vector<vector<Mat>> &costs) {
  Size dispSize = disparity.size();
  ...
  Canny(detectedEdges, detectedEdges, cannyThreshold1, cannyThreshold2,
        cannyKernelSize);
  ...
  for (size_t h = 1; h < dispSize.height - 1; h++) {
    for (size_t w = 1; w < dispSize.width - 1; w++) {
      ...
      if (detectedEdges.at<uchar>(h, w) != 0) {
        int direction = -1;
        if (detectedEdges.at<uchar>(h - 1, w - 1) != 0 &&
            detectedEdges.at<uchar>(h + 1, w + 1) != 0) {
          direction = 0;
        } else if (detectedEdges.at<uchar>(h - 1, w + 1) != 0 &&
                   detectedEdges.at<uchar>(h + 1, w - 1) != 0) {
          direction = 4;
        } else if (detectedEdges.at<uchar>(h - 1, w) != 0 ||
                   detectedEdges.at<uchar>(h + 1, w) != 0) {
          if (detectedEdges.at<uchar>(h - 1, w - 1) != 0 ||
              detectedEdges.at<uchar>(h - 1, w) != 0 ||
              detectedEdges.at<uchar>(h - 1, w + 1) != 0)
            if (detectedEdges.at<uchar>(h + 1, w - 1) != 0 ||
          ...
          if (detectedEdges.at<uchar>(h - 1, w - 1) != 0 ||
              detectedEdges.at<uchar>(h, w - 1) != 0 ||
              detectedEdges.at<uchar>(h + 1, w - 1) != 0)
            if (detectedEdges.at<uchar>(h - 1, w + 1) != 0 ||
                detectedEdges.at<uchar>(h, w + 1) != 0 ||
                detectedEdges.at<uchar>(h + 1, w + 1) != 0)
        ...
        if (direction != -1) {
          ...
          if (disp >= dMin) {
            ...
            int d1 = disparity.at<int>(h + directionsH[direction],
                                       w + directionsW[direction]);
            int d2 = disparity.at<int>(h + directionsH[direction + 1],
                                       w + directionsW[direction + 1]);

            costType cost1 = (d1 >= dMin) ? costs[0][d1 - dMin].at<costType>(
                                                h + directionsH[direction],
                                                w + directionsW[direction])
                                          : -1;

            costType cost2 = (d2 >= dMin) ? costs[0][d2 - dMin].at<costType>(
                                                h + directionsH[direction + 1],
                                                w + directionsW[direction + 1])
                                          : -1;
...
Mat DisparityRefinement::subpixelEnhancement(Mat &disparity,
  ...
  for (size_t h = 0; h < dispSize.height; h++) {
    for (size_t w = 0; w < dispSize.width; w++) {
      ...
      if (disp > dMin && disp < dMax) {
        float cost =
            costs[0][disp - dMin].at<costType>(h, w) / (float)COST_FACTOR;
        float costPlus =
            costs[0][disp + 1 - dMin].at<costType>(h, w) / (float)COST_FACTOR;
        float costMinus =
            costs[0][disp - 1 - dMin].at<costType>(h, w) / (float)COST_FACTOR;

        float diff =
            (costPlus - costMinus) / (2 * (costPlus + costMinus - 2 * cost));
...
Mat DisparityRefinement::convertDisp2Gray(const Mat &disparity) {
  ...
  for (size_t h = 0; h < dispSize.height; h++) {
    for (size_t w = 0; w < dispSize.width; w++) {
      dispU.at<uchar>(h, w) =
          (disparity.at<int>(h, w) < 0) ? 0 : (uchar)disparity.at<int>(h, w);
    }
  }
